    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="margin-bottom">
                <a class="btn btn-primary" href="{{ adminUrl('study-levels/create') }}">
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ trans('form.list_of',['name' => trans_choice('label.study_level_lc', 2)]) }}</h3>
                </div><!-- /.box-header -->
                @if($study_levels->count()>0)
                            ...
                            <tbody>
                                @foreach($study_levels as $study_level)
                                    <tr>
                                        ...
                                        <td>{{ $study_level->description }}</td>
                                        <td>{{ $study_level->order }}</td>
                                        <td>
                                            <a href="{{ adminUrl('study-levels/{id}/edit', ['id'=> $study_level->id]) }}">
                                                {{ trans('form.action_edit') }}
                                            </a>
                                            <a class="delete" href="{{ addRdrUrl(adminUrl('study-levels/{id}', ['id'=> $study_level->id])) }}">
