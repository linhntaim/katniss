...
@section('page_title', trans('pages.admin_students_title'))
@section('page_description', trans('pages.admin_students_desc'))
@section('page_breadcrumb')
<ol class="breadcrumb">
    <li><a href="{{ adminUrl() }}"><i class="fa fa-home"></i> {{ trans('pages.admin_dashboard_title') }}</a></li>
    <li><a href="{{ $redirect_url }}">{{ trans('pages.admin_students_title') }}</a></li>
...
@section('page_content')
    <form method="post" action="{{ adminUrl('students/{id}', ['id' => $student->user_id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addRdrUrl(addErrorUrl(adminUrl('students/{id}', ['id'=> $student->user_id])), $redirect_url) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    @if(!$student->isApproved)
                        <a class="btn btn-success approve" href="{{ addRdrUrl(addErrorUrl(adminUrl('students/{id}', ['id'=> $student->user_id]) . '?approve=1')) }}">
                            {{ trans('form.action_approve') }}
                        </a>
                    @else
                        <a class="btn btn-danger reject" href="{{ addRdrUrl(addErrorUrl(adminUrl('students/{id}', ['id'=> $student->user_id]) . '?reject=1')) }}">
                            {{ trans('form.action_reject') }}
                        </a>
                    @endif
                    <a class="btn btn-primary pull-right" href="{{ addRdrUrl(adminUrl('students/create'), $redirect_url) }}">
                        {{ trans('form.action_add') }} {{ trans_choice('label.student_lc', 1) }}
                    </a>
                </div>
                <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.student_lc', 1) }}</h4>
                @if (count($errors) > 0)
