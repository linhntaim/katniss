...
@section('page_content')
    <form class="check-slug" method="post" action="{{ adminUrl('media-categories/{id}', ['id'=> $category->id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addErrorUrl(adminUrl('media-categories/{id}', ['id'=> $category->id])) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    <a class="btn btn-success" href="{{ adminUrl('media-categories/{id}/sort', ['id' => $category->id]) }}">
                        {{ trans('form.action_sort') }}
                    </a>
                    <a class="btn btn-primary pull-right" href="{{ adminUrl('media-categories/create') }}">
