...
@section('page_content')
    <form method="post" action="{{ adminUrl('study-problems/{id}', ['id'=> $study_problem->id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addErrorUrl(adminUrl('study_problems/{id}', ['id'=> $study_problem->id])) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    <a class="btn btn-primary pull-right" href="{{ adminUrl('study-problems/create') }}">
                        {{ trans('form.action_add') }} {{ trans_choice('label.study_problem_lc', 1) }}
                    </a>
                </div>
                <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.study_problem_lc', 1) }}</h4>
                @if (count($errors) > 0)
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" id="inputOrder" name="order"
                           placeholder="{{ trans('label.sort_order') }}" type="text" value="{{ $study_problem->order }}">
                </div>
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        @foreach(supportedLocalesAsInputTabs() as $locale => $properties)
                            <?php
                            $trans = $study_problem->translate($locale);
