...
@section('page_content')
    <form method="post" action="{{ adminUrl('professional-skills/{id}', ['id'=> $professional_skill->id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addErrorUrl(adminUrl('professional_skills/{id}', ['id'=> $professional_skill->id])) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    <a class="btn btn-primary pull-right" href="{{ adminUrl('professional-skills/create') }}">
                        {{ trans('form.action_add') }} {{ trans_choice('label.professional_skill_lc', 1) }}
                    </a>
                </div>
                <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.professional_skill_lc', 1) }}</h4>
                @if (count($errors) > 0)
                ...
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        @foreach(supportedLocalesAsInputTabs() as $locale => $properties)
                            <?php
                            $trans = $professional_skill->translate($locale);
                ...
                <div>
                    ...
                    <div class="pull-right">
                        ...
                        <a role="button" class="btn btn-warning" href="{{ adminUrl('professional-skills') }}">{{ trans('form.action_cancel') }}</a>
                    </div>
                </div>
            </div>
        </div>
