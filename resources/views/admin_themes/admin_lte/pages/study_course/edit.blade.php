...
@section('page_content')
    <form method="post" action="{{ adminUrl('study-courses/{id}', ['id'=> $study_course->id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addErrorUrl(adminUrl('study_courses/{id}', ['id'=> $study_course->id])) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    <a class="btn btn-primary pull-right" href="{{ adminUrl('study-courses/create') }}">
                        {{ trans('form.action_add') }} {{ trans_choice('label.study_course_lc', 1) }}
                    </a>
                </div>
                <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.study_course_lc', 1) }}</h4>
                @if (count($errors) > 0)
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" id="inputOrder" name="order"
                           placeholder="{{ trans('label.sort_order') }}" type="text" value="{{ $study_course->order }}">
                </div>
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        @foreach(supportedLocalesAsInputTabs() as $locale => $properties)
                            <?php
                            $trans = $study_course->translate($locale);
                ...
                <div>
                    ...
                    <div class="pull-right">
                        ...
                        <a role="button" class="btn btn-warning" href="{{ adminUrl('study-courses') }}">{{ trans('form.action_cancel') }}</a>
                    </div>
                </div>
            </div>
        </div>
