...
@section('page_content')
    <form method="post" action="{{ adminUrl('announcements/{id}', ['id'=> $announcement->id]) }}">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="row">
            <div class="col-xs-12">
                <div class="margin-bottom">
                    <a class="btn btn-warning delete" href="{{ addErrorUrl(adminUrl('announcements/{id}', ['id'=> $announcement->id])) }}">
                        {{ trans('form.action_delete') }}
                    </a>
                    <a class="btn btn-primary pull-right" href="{{ adminUrl('announcements/create') }}">
                        {{ trans('form.action_add') }} {{ trans_choice('label.announcement_lc', 1) }}
                    </a>
                </div>
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.announcement_lc', 1) }}</h4>
                    </div>
                    <div class="box-body">
                        @if (count($errors) > 0)
                        ...
                        @endif
                            <div class="form-group">
                                ...
                                <input type="text" placeholder="{{ trans('label.title') }}" value="{{ $announcement->title }}"
                            ...
                            <div class="form-group">
                                ...
                                <textarea placeholder="{{ trans('label.content') }}" class="form-control" rows="10" cols="5"
                                          id="inputTitle" name="content" required>{{ $announcement->content }}</textarea>
                            </div>
                            <hr>
                            <div class="form-group">
                                <div class="checkbox icheck">
                                    <label for="inputToAll">
                                        <input id="inputToAll" name="to[all]" type="checkbox"
                                               value="1"{{ $announcement->to_all == 1 ? ' checked' : '' }}>
                                        &nbsp; {{ trans('label.to_all') }}
                                    </label>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <select id="inputToRoles" class="form-control select2" name="to[roles][]" multiple style="width: 100%"
                                    ...
                                    @foreach($roles as $role)
                                        <option value="{{ $role->id }}"{{ in_array($role->id, $announcement->toRoles) ? ' selected' : '' }}>
                                            {{ $role->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            @if(!empty($announcement->toUsers))
                                <div class="form-group">
                                    <label class="control-label">
                                        {{ trans('label._current', ['current' => trans_choice('label.user', count($announcement->toUsers))]) }}
                                    </label>
                                    @foreach($announcement->toUsers as $user)
                                        <div class="checkbox">
                                            <input id="inputUser-{{ $user }}" name="existed_ids[]" type="checkbox"
                                                   value="{{ $user }}" checked>
                                            &nbsp; #{{ $user }}
                                        </div>
                                    @endforeach
